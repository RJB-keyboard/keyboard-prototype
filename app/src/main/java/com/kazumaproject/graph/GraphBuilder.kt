...
class GraphBuilder {
    ...
    fun constructGraph(
        ...
        for (i in str.indices){
            ...
            // Retain an explicit high-cost unknown-kana path, as a normal IME does.
            // It is never presented as a successful dictionary match.
            graph[i + 1].add(mutableListOf(Node(
                l = 0, r = 0, score = 20000, f = 20000,
                tango = str[i].toString(), len = 1, sPos = i,
                key = str[i].toString(), wcost = 20000,
            )))
        }
        return graph
    }
