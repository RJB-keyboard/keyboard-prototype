...
句読点キーはタップで「、」、左フリックで「。」、上で「！」、右で「？」、下で「...」（半角ピリオド3つ）を直接入力します。斜めは移動量の大きい軸で判定します。操作中はキー上に選択中の文字を表示し、指を離すと確定します。
...
軌跡から少し外れた文字も辞書・文脈で選べるよう、各通過点ではキー幅・高さで正規化した距離1.5以内の最大9キーを探索します。移動中の距離による減点も緩め、タップやキー中心での滞留は従来の位置精度を保ちます。近い3キーだけに限定していたときに落ちていた上下左右・斜めの文字を含めつつ、離れたキーは対象外とします。詳細と合成軌跡による検証は [探索エンジン](docs/engine.md) を参照してください。
...
- 句読点キー「、。」はタップで「、」、左で「。」、上で「！」、右で「？」、下で「...」を入力します。
...
候補が表示されたら右上の「⌄」で一覧を五十音キーの領域まで広げられます。展開中は縦にスクロールして候補を選べます。「⌃」で候補を保持したまま五十音キーに戻ります。候補確定・削除・スペース・改行・カーソル移動・入力先の切り替えでは通常表示に戻ります。キーボード全体の高さとOSボタン用の余白は変わりません。表示するのは既存エンジンが返した候補のみで、生成件数は変更していません。

句読点キーを押している間は、キーの上に十字型のフリック案内（中央「、」・左「。」・上「！」・右「？」・下「...」）を表示します。選択中の記号は青背景・白文字で強調し、指を離す・キャンセル・キーボードを閉じると案内を消します。
