...
候補が表示されたら右上の「⌄」で一覧を五十音キーの領域まで広げられます。展開中は18spの文字と左右4dpの余白で、候補の文字幅に合わせて左から詰めて表示します。行に収まらない候補は次の行へ送り、画面幅を超える長い候補は候補内で折り返します。タップ領域は最低48dpを確保し、縦にスクロールして候補を選べます。「⌃」で候補を保持したまま五十音キーに戻ります。候補確定・削除・スペース・改行・カーソル移動・入力先の切り替えでは通常表示に戻ります。キーボード全体の高さとOSボタン用の余白は変わりません。表示するのは既存エンジンが返した候補のみで、生成件数は変更していません。
