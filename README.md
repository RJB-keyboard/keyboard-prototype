...
表は右から左に「あ・か・さ・た・な・は・ま・や・ら・わ」の10列で、各列は上から下に並びます。わ行は「わ・空白・を・空白・ん」を縦一列に配置します。現代仮名46文字と「っ」「ー」の48キーを配置します。や行は「や・っ・ゆ・ー・よ」、わ行の空きマスはキーにしません。グライド中は線と現在のキーを強調表示します。
...
テストでは、Aの正規化・通過点・繰り返し・濁音／小書き文字、Bによる候補順位の変化、混合確率、異常な入力、中断、古い結果の破棄、実辞書の漢字変換と読み戻しを確認します。端末テストでは実ONNXモデルの分布・文脈依存・バッチ整合性と、48キーと空きマスのヒット判定、既存のタッチ・候補選択・キャンセル動作も検証します。
